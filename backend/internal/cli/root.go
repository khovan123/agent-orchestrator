...
func (d Deps) withDefaults() Deps {
	def := DefaultDeps()
	if d.In == nil { d.In = def.In }
	if d.Out == nil { d.Out = def.Out }
	if d.Err == nil { d.Err = def.Err }
	if d.HTTPClient == nil { d.HTTPClient = def.HTTPClient }
	if d.Executable == nil { d.Executable = def.Executable }
	if d.StartProcess == nil { d.StartProcess = def.StartProcess }
	if d.ProcessAlive == nil { d.ProcessAlive = def.ProcessAlive }
	if d.LookPath == nil { d.LookPath = def.LookPath }
	if d.CommandOutput == nil { d.CommandOutput = def.CommandOutput }
	if d.CommandOutputInDir == nil { d.CommandOutputInDir = def.CommandOutputInDir }
	if d.RunInteractiveCommand == nil { d.RunInteractiveCommand = def.RunInteractiveCommand }
	if d.ReadSecret == nil { d.ReadSecret = def.ReadSecret }
	if d.DoctorGitHubRESTBase == "" { d.DoctorGitHubRESTBase = def.DoctorGitHubRESTBase }
	if d.DoctorGitLabRESTBase == "" { d.DoctorGitLabRESTBase = def.DoctorGitLabRESTBase }
	if d.Now == nil { d.Now = def.Now }
	if d.Sleep == nil { d.Sleep = def.Sleep }
	return d
}
...
func NewRootCommand(deps Deps) *cobra.Command {
	...
	root := &cobra.Command{
		...
		PersistentPreRunE: func(cmd *cobra.Command, _ []string) error {
			if shouldEmitCLIInvocation(cmd) { ctx.emitCLIInvoked(cmd.Context(), cmd) }
			return nil
		},
	}
	root.SetIn(deps.In)
	...
	root.AddCommand(newAgyTUIHookCommand(ctx))
...
type commandContext struct { deps Deps }

func shouldEmitCLIInvocation(cmd *cobra.Command) bool {
	commandPath := telemetrymeta.NormalizeCommandPath(cmd.CommandPath())
	if telemetrymeta.IsRoutineInternalCLICommand(commandPath) { return false }
	switch commandPath {
	case "ao daemon", "ao start", "ao completion", "ao help", "ao pty-host", "ao chat-host", "ao codex-login", "ao agent-process", "ao agent-process supervise", "ao agy-tui-hook", "ao agy-chat-hook":
		return false
	default:
		return true
	}
...
func (c *commandContext) emitCLIInvoked(ctx context.Context, cmd *cobra.Command) {
	...
	_ = c.postLoopbackJSON(reqCtx, "/internal/telemetry/cli-invoked", map[string]string{
		"command": cmd.Name(), "commandPath": cmd.CommandPath(), "actorType": cliInvocationActorType(cmd),
	})
}

func cliInvocationActorType(cmd *cobra.Command) string {
	if strings.TrimSpace(cmd.CommandPath()) == "ao hooks" { return "agent" }
	if sessionIDPattern.MatchString(strings.TrimSpace(os.Getenv("AO_SESSION_ID"))) { return "agent" }
	return "user"
}
...
func (c *commandContext) emitCLIUsageError(ctx context.Context, args []string, err error) {
	...
	_ = c.postLoopbackJSON(reqCtx, "/internal/telemetry/cli-usage-error", map[string]string{
		"command": command, "commandPath": commandPath, "error": err.Error(),
	})
}
...
func usageErrorCommand(args []string) (string, string) {
	...
	for _, arg := range args {
		...
		for _, sub := range current.Commands() {
			if sub.Name() == arg || sub.HasAlias(arg) { next = sub; break }
		}
		if next == nil { break }
	...
	command := "ao"
	if len(tokens) > 1 { command = tokens[len(tokens)-1] }
	return command, commandPath
}

func usageArgs(validate cobra.PositionalArgs) cobra.PositionalArgs {
	return func(cmd *cobra.Command, args []string) error {
		if err := validate(cmd, args); err != nil { return usageError{err} }
		return nil
	}
}

func noArgs(cmd *cobra.Command, args []string) error {
	if err := cobra.ExactArgs(0)(cmd, args); err != nil { return usageError{err} }
	return nil
}

func atMostOneArg(cmd *cobra.Command, args []string) error {
	if err := cobra.MaximumNArgs(1)(cmd, args); err != nil { return usageError{err} }
	return nil
}

func newDaemonCommand() *cobra.Command {
	return &cobra.Command{Use: "daemon", Short: "Run the AO backend daemon", Hidden: true, Args: noArgs, RunE: func(cmd *cobra.Command, args []string) error { return daemon.Run() }}
}
